...
def render_trading_card(pid, pname, tid, tname, theme, df, badges):
    if df.empty: return
    
    primary = theme[0]
    img_url = f"https://cdn.nba.com/headshots/nba/latest/1040x760/{pid}.png" if pid else f"https://cdn.nba.com/logos/nba/{tid}/global/L/logo.svg"
    ...
    st.markdown(f"""
<style>
.card-container {{ perspective: 1000px; width: 100%; max-width: 350px; margin: 0 auto; padding: 20px 0; }}
.tcg-card {{ position: relative; width: 100%; aspect-ratio: 2.5 / 3.5; border-radius: 18px; background: linear-gradient(135deg, #111 0%, #222 100%); box-shadow: 0 15px 35px rgba(0,0,0,0.5), inset 0 0 0 2px rgba(255,255,255,0.2), inset 0 0 0 6px {primary}; overflow: hidden; transition: transform 0.3s ease, box-shadow 0.3s ease; transform-style: preserve-3d; }}
.tcg-card:hover {{ transform: translateY(-10px) rotateX(5deg) rotateY(-5deg); box-shadow: -10px 25px 40px rgba(0,0,0,0.6), inset 0 0 0 2px rgba(255,255,255,0.4), inset 0 0 0 6px {primary}; }}
.tcg-card::before {{ content: ""; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(115deg, transparent 20%, rgba(255,255,255,0.3) 30%, transparent 40%, rgba(255,255,255,0.1) 50%, transparent 60%); background-size: 200% 200%; mix-blend-mode: color-dodge; opacity: 0; z-index: 10; transition: opacity 0.3s ease; pointer-events: none; }}
.tcg-card:hover::before {{ opacity: 1; animation: foil-shimmer 2.5s infinite linear; }}
@keyframes foil-shimmer {{ 0% {{ background-position: 200% 0; }} 100% {{ background-position: -200% 0; }} }}
.tcg-art {{ position: absolute; top: 10%; left: 5%; width: 90%; height: 50%; background-image: radial-gradient(circle at center, {hex_to_rgba(primary, 0.4)} 0%, transparent 70%); display: flex; justify-content: center; align-items: flex-end; border-bottom: 2px solid {primary}; }}
.tcg-art img {{ height: 110%; object-fit: cover; filter: drop-shadow(0 10px 15px rgba(0,0,0,0.8)); }}
.tcg-header {{ position: absolute; top: 0; width: 100%; padding: 12px 16px; display: flex; justify-content: space-between; align-items: center; background: linear-gradient(to bottom, rgba(0,0,0,0.8) 0%, transparent 100%); z-index: 2; }}
.tcg-name {{ font-family: 'Barlow Condensed', sans-serif; font-size: 22px; font-weight: 800; text-transform: uppercase; color: white; text-shadow: 2px 2px 0px #000; }}
.tcg-hp {{ font-family: 'DM Mono', monospace; font-size: 14px; font-weight: 700; color: #fff; }}
.tcg-hp span {{ color: {primary}; font-size: 20px; }}
.tcg-body {{ position: absolute; bottom: 0; width: 100%; height: 40%; background: rgba(10,10,10,0.95); border-top: 2px solid rgba(255,255,255,0.1); padding: 12px; display: flex; flex-direction: column; justify-content: space-between; }}
.tcg-badges {{ font-family: 'DM Mono', monospace; font-size: 9px; line-height: 1.2; }}
.tcg-stats-row {{ display: flex; justify-content: space-between; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 8px; }}
...
.tcg-footer {{ text-align: center; font-family: 'DM Mono', monospace; font-size: 7px; color: rgba(255,255,255,0.2); margin-top: 4px; }}
</style>
...
with st.sidebar:
    st.markdown(f"<div style=\"font-family:'Barlow Condensed',sans-serif; font-size:22px; font-weight:800; text-transform:uppercase; color:white; letter-spacing:1px; margin-bottom:16px;\">NBA Shot Lab<span style=\"font-family:'DM Mono',monospace; font-size:10px; color:rgba(255,255,255,0.3); vertical-align:middle; margin-left:6px;\">v10</span></div>", unsafe_allow_html=True)
...
def render_player_dashboard(pid, tid, pname, tname, theme, key_prefix, sel_game, lbl_display, opp_display):
    ...
    col_img, col_hdr1, col_hdr2 = st.columns([1, 4, 2])
    with col_img:
        st.markdown(f"""<img src="{img_url}" style="width:72px; height:72px; border-radius:50%; border:2px solid {theme[0]}; object-fit:contain; background:rgba(0,0,0,0.3); padding:5px; box-shadow: 0 0 28px {hex_to_rgba(theme[0], 0.3)};">""", unsafe_allow_html=True)
    with col_hdr1:
        st.markdown(f"""
